...
{
    Vector3 normalizeCameraPosition;
    float currentZoomLevel;
    public PerspectiveZoomStrategy(Camera cam, Vector3 offset, float startingZoom)
    {
        normalizeCameraPosition = new Vector3(0f, Mathf.Abs(offset.y), -Mathf.Abs(offset.x)).normalized;
    ...
    {
        cam.transform.localPosition = normalizeCameraPosition * currentZoomLevel;
    }
